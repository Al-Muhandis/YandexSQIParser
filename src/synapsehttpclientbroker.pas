...
type
  ...
  public
    ...
    procedure Get(const AUrl: String; aStream: TStream); override;
...
procedure TSynapseHTTPClient.Get(const AUrl: String; aStream: TStream);
begin
  BeforeRequest;
  if FHTTPClient.ResultCode=200 then
    FHTTPClient.Document.SaveToStream(aStream);
  FHTTPClient.HTTPMethod('GET', AUrl);
  FHTTPClient.Document.SaveToStream(aStream);
  if FHTTPClient.ResultCode<>200 then
    raise EHTTPClient.Create('HTTP client. '+FHTTPClient.ResultCode.ToString+' '+
      FHTTPClient.ResultString);
